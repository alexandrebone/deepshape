...
if __name__ == '__main__':
    ...
    for epoch in range(number_of_epochs + 1):
        ...
        for k in range(number_of_images_train // batch_size):  # drops the last batch
            ...
            ss_s_var = torch.mean(batch_latent__s ** 2).cpu().detach().numpy()
            ss_a_var = torch.mean(batch_latent__a ** 2).cpu().detach().numpy()
            ...
            kullback_regularity_loss__s = torch.sum(batch_latent__s.pow(2)) / lambda_square__s
            train_kullback_regularity_loss__s = kullback_regularity_loss__s.detach().cpu().numpy()

            kullback_regularity_loss__a = torch.sum(batch_latent__a.pow(2)) / lambda_square__a
        ...
        if epoch % save_every_n_iters == 0 or epoch == number_of_epochs:
            ...
            np.save(os.path.join(output_dir, 'det_init__s__epoch_%d.npy' % epoch), model.s.detach().cpu().numpy())
            np.save(os.path.join(output_dir, 'det_init__a__epoch_%d.npy' % epoch), model.a.detach().cpu().numpy())

            n = 5
            # model.write(intensities[:n], os.path.join(output_dir, 'train__epoch_%d' % epoch),
            #             intensities_mean, intensities_std)
            model.write(intensities[:n], model.s[:n], model.a[:n],
