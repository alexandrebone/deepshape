...
class BayesianAtlas(nn.Module):
    ...
    def __init__(self,
        ...
        self.template_points = template_points.requires_grad_()
        ...
        if self.dimension == 2:
            self.encoder = Encoder2d(self.splatting_grid_size, latent_dimension)
            self.decoder = Decoder2d(latent_dimension, deformation_grid_size)
        elif self.dimension == 3:
            self.encoder = Encoder3d(self.splatting_grid_size, latent_dimension)
    ...
    def update_template(self, kernel, gamma, lr):
        self.template_points = self.template_points.detach() - lr * kernel(
...
if __name__ == '__main__':
    ...
    if dataset in ['circles', 'ellipsoids', 'starmen']:
        ...
        if 'cuda' in device:
            initial_hippocampus_p = initial_hippocampus_p.cuda()
            initial_hippocampus_c = initial_hippocampus_c.cuda()
    ...
    for epoch in range(number_of_epochs + 1):
        ...
        if epoch % save_every_n_iters == 0:
            ...
            elif dataset in ['ellipsoids', 'hippocampi']:
                model.write_meshes(
