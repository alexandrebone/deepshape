...
class BayesianAtlas(nn.Module):
    ...
    def forward(self, q, with_regularity_loss=False):
        ...
        x = torch.stack(torch.meshgrid(
            [torch.linspace(0.0, gs - 1.0, dgs),
             torch.linspace(0.0, gs - 1.0, dgs),
             torch.linspace(0.0, gs - 1.0, dgs)])).type(str(q.type())).view(1, 3, dgs, dgs, dgs).repeat(bts, 1, 1, 1, 1)
        for v in v_t:
            x += self.dt * batched_vector_interpolation(v, x, dsf)
    ...
    def tamper_template_gradient(self, lr, print_info=False):
        ...
        if print_info:
            print(torch.min(tampered_template_gradient), torch.max(tampered_template_gradient))
    ...
    def write_trajectories(self, observations, prefix):
        ...
        write_images(batched_scalar_interpolation(self.template_intensities, x).detach().cpu().numpy(),
