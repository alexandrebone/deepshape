...
class BayesianAtlas(nn.Module):
    ...
    def __init__(self,
        ...
        self.hamiltonian = Hamiltonian(2 * latent_dimension_half)
    ...
    def forward(self, q_0):
        ...
        # DECODE
        q_0_norm = torch.norm(q_t[0], p=2, dim=1)
        v_t = []
        for (p, q) in zip(p_t, q_t):
            q_t_norm = torch.norm(q, p=2, dim=1)
            v_t.append(self.decoder(q * (q_0_norm / q_t_norm).view(-1, 1).expand(q.size())))
...
experiment_prefix = '16__bayesian_atlas'
...
template_connectivity = connectivities[0]

noise_variance = 1.0
noise_dimension = points.size(1)

model = BayesianAtlas(torch.mean(points, dim=0), template_connectivity,
                      latent_dimension_half, splatting_grid_size, deformation_grid_size,
                      deformation_kernel_width, number_of_time_points)
...
for epoch in range(number_of_epochs + 1):
    ...
    train_attachment_loss /= float(batch_size * (number_of_starmen // batch_size))
    train_regularity_loss /= float(batch_size * (number_of_starmen // batch_size))
    train_total_loss /= float(batch_size * (number_of_starmen // batch_size))
    ...
    if epoch % print_every_n_iters == 0:
        print('[Epoch: %d] Noise std = %.2E ; Train loss = %.3f (attachment = %.3f ; regularity = %.3f)' %
              (epoch, math.sqrt(noise_variance), train_total_loss, train_attachment_loss, train_regularity_loss))
    ...
    if epoch % save_every_n_iters == 0:
        ...
        n = 3
        model.write(splats[:n].permute(0, 3, 1, 2), points[:n], connectivities[:n], visualization_grid,
                    os.path.join(output_dir, 'epoch_%d' % epoch))
