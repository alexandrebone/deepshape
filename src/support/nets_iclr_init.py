...
class MetamorphicAtlas(nn.Module):
    ...
    def transform(self, v_star, n_star):

        # INIT
        bts = v_star.size(0)
        assert bts == n_star.size(0)
        ntp = self.number_of_time_points
        kws = self.kernel_width__s
        kwa = self.kernel_width__a
        dim = self.dimension
        ...
        grid = torch.stack(torch.meshgrid(
            [torch.linspace(0.0, gs - 1.0, dgs),
             torch.linspace(0.0, gs - 1.0, dgs)])).type(str(v_star.type())).view(1, 2, dgs, dgs).repeat(bts, 1, 1, 1)
        ...
        return intensities, v_norm_squared, n_norm_squared
    ...
    def write_transform(self, observations, v_star, n_star, prefix, mean, std):
