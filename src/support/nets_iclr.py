...
class Encoder2d__final(nn.Module):
    ...
    def __init__(self, in_grid_size, latent_dimension__s, latent_dimension__a, init_var__s=1.0, init_var__a=1.0):
        ...
        self.linear_mean_1__s = Linear_Tanh(16 * n * n, 8 * n * n, bias=False)
        self.linear_mean_2__s = nn.Linear(8 * n * n, latent_dimension__s, bias=False)
        self.linear_logv_1__s = Linear_Tanh(16 * n * n, 8 * n * n, bias=False)
        self.linear_logv_2__s = nn.Linear(8 * n * n, latent_dimension__s, bias=False)

        self.linear_mean_1__a = Linear_Tanh(16 * n * n, 8 * n * n, bias=False)
        self.linear_mean_2__a = nn.Linear(8 * n * n, latent_dimension__a, bias=False)
        self.linear_logv_1__a = Linear_Tanh(16 * n * n, 8 * n * n, bias=False)
...
class DeepDecoder2d__smaller(nn.Module):
    ...
    def __init__(self, latent_dimension, out_channels, out_grid_size):
        ...
        print(
            '>> DeepDecoder2d__smaller has {} parameters'.format(sum([len(elt.view(-1)) for elt in self.parameters()])))
...
class DeepDecoder2d__Unique(nn.Module):
    """
    in: latent_dimension
    out: out_grid_size * out_grid_size * 3
    """
    ...
    def __init__(self,
                 latent_dimension_s, out_channels_s,
                 latent_dimension_a, out_channels_a, out_grid_size):
        nn.Module.__init__(self)
        self.inner_grid_size = int(out_grid_size * 2 ** -4)
        self.out_channels_s = out_channels_s
        self.out_channels_a = out_channels_a
        self.linear1 = Linear_Tanh(latent_dimension_s + latent_dimension_a, 16 * self.inner_grid_size ** 2, bias=False)
        self.linear2 = Linear_Tanh(16 * self.inner_grid_size ** 2, 32 * self.inner_grid_size ** 2, bias=False)
        # self.linear3 = Linear_Tanh(16 * self.inner_grid_size ** 2, 16 * self.inner_grid_size ** 2, bias=False)
        self.up1 = ConvTranspose2d_Tanh(32, 16, bias=False)
        self.up2 = ConvTranspose2d_Tanh(16, 8, bias=False)
        self.up3 = ConvTranspose2d_Tanh(8, 4, bias=False)
        self.up4 = ConvTranspose2d_Tanh(4, out_channels_s + out_channels_a * 2, bias=False)
        self.up5 = ConvTranspose2d_Tanh(out_channels_a * 2, out_channels_a, bias=False)
        print(
            '>> DeepDecoder2d__Unique has {} parameters'.format(sum([len(elt.view(-1)) for elt in self.parameters()])))
    ...
    def forward(self, s, a):
        ...
        x = self.up3(x)
        x = self.up4(x)
        return x[:, :self.out_channels_s], self.up5(x[:, self.out_channels_s:])
...
class MetamorphicAtlas(nn.Module):
    ...
    def __init__(self, template_intensities, number_of_time_points, downsampling_factor,
        ...
        # self.x_average = torch.stack(torch.meshgrid([torch.linspace(0.0, gs - 1.0, dgs),
        #                                              torch.linspace(0.0, gs - 1.0, dgs)])).view(1, 2, dgs, dgs)
        # self.v_average = torch.zeros((1, gs, gs))
    ...
    def encode(self, x):
        ...
        # means__s, log_variances__s, means__a, log_variances__a = self.encoder(x)
        # mean__st, log_variance__st, mean__at, log_variance__at = self.encoder(self.template_intensities.detach().unsqueeze(0))
        # return (means__s - mean__st.detach().expand(means__s.size()), log_variances__s,
        #         means__a - mean__at.detach().expand(means__a.size()), log_variances__a)
    ...
    def decode(self, s, a):
        ...
        # # L1 REG
        # s_norm_squared = torch.sum(s ** 2, dim=1)
        # a_norm = torch.sum(torch.abs(a) ** 2, dim=1)
        # v_norm_squared = torch.sum(v * v_star, dim=tuple(range(1, dim + 2)))
        # n_norm = torch.sqrt(torch.sum(n * n_star, dim=tuple(range(1, dim + 2))))
        # normalizer__s = torch.where(s_norm_squared > 1e-10,
        #                             torch.sqrt(s_norm_squared / v_norm_squared),
        #                             torch.from_numpy(np.array(0.0)).float().type(str(s.type())))
        # normalizer__a = torch.where(a_norm > 1e-5,
        #                             a_norm / n_norm,
        #                             torch.from_numpy(np.array(0.0)).float().type(str(a.type())))
        ...
        # INTERPOLATE
        # intensities = batched_scalar_interpolation(
        #     self.template_intensities + (n - self.n_average.type(str(n.type()))),
        #     x - self.x_average.type(str(x.type())))
    ...
    # def update_template(self, observations):
    ...
    #
    #     # UPDATE
    #     self.template_intensities.data = torch.mean(self.template_intensities + n, dim=0).detach()

    def update_fixed_effects(self, observations):
        s, _, a, _ = self.encode(observations)
        ...
        self.v_star_average = torch.mean(v_star.detach(), dim=0)
        self.n_star_average = torch.mean(n_star.detach(), dim=0)
        ...
        # UPDATE
        x_average = torch.mean(x.detach(), dim=0)
        n_average = torch.mean(n.detach(), dim=0)
        self.template_intensities.data = batched_scalar_interpolation(
            self.template_intensities.detach() + n_average, x_average.unsqueeze(0))[0]
